...
$\mathbf{a}_h  = [a_{hx},\, a_{hy},\, a_{hz}]$ & 经重映射后的佩戴者头部坐标系三轴加速度 （m/s$^2$）\\
$\boldsymbol{\omega}_h = [\omega_{hx},\, \omega_{hy},\, \omega_{hz}]$ & 经重映射后的佩戴者头部坐标系三轴角速度 （dps）\\
$\theta$（pitch） & 头部俯仰角，单位：度（$^{\circ}$） \\
$\phi$（roll） & 头部横滚角，单位：度（$^{\circ}$） \\
$\psi$（yaw） & 头部侧倾角，单位：度（$^{\circ}$） \\
