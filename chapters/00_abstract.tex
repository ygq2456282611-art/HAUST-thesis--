...
{\zihao{-4}\songti
脑卒中是全球范围内导致长期残疾的首要原因之一。60\%至73\%的脑卒中幸存者伴有不同程度的视觉功能障碍，包括同向性偏盲、眼球运动障碍和单侧空间忽略等，这些视觉问题严重影响患者的日常生活能力与康复预后~\cite{rowe2021ivis,liu2023vision,zheng2025stroke,sun2024pursuit}。然而，现有康复手段面临"两端不足"的困境：传统人工训练缺乏标准化和量化记录，VR/AR等高端设备成本高昂、操作复杂，难以满足居家长期康复需求。针对这一空白，本文设计并实现了一种基于STM32H723VGT6微控制器的低成本头戴式眼部康复训练装置。

装置以DM-MC02核心板为硬件平台，集成了BMI088六轴惯性传感器、SG90舵机、650nm红色激光模块、基于CI1302芯片的语音交互模块以及WS2812状态指示灯等元件。在软件层面，基于四元数融合框架实现了多策略姿态估计算法，通过传感器坐标系与头部坐标系的直接映射和100帧滑动方差头稳指标实现了佩戴者头部姿态的实时10ms级分析。开发了一个包含九种状态的系统状态机，支持完整训练模式（五种模式A至E自动串联）和自选训练模式（语音命令逐一选择）两种工作方式。五种临床训练模式——注视稳定性训练（15秒）、扫视训练（8次随机跳变）、平稳追踪训练（8点平滑移动）、视觉聚焦训练（5循环远近交替）和空间忽略训练（6次左右交替）——覆盖了从被动注视到主动视觉搜索的渐进式康复体系。语音交互系统基于离线识别引擎，支持10条可识别命令词和46条TTS播报词，配合三级安全保护机制（姿态超限自动暂停、语音紧急中断和跨状态模式切换）保证了训练过程的连贯性和操作安全性。
